...
public class ResTenPayment extends PageUtils{

	public JFrame rtpFrame;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ResTenPayment window = new ResTenPayment(null);
					window.rtpFrame.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	 ...
	 */
	public ResTenPayment(String restenname) {
	...
	private void initialize(String restenname) {
		rtpFrame = new JFrame();
		rtpFrame.setTitle("Resident/Tenant Payment");
		rtpFrame.setBounds(100, 100, 900, 700);
		rtpFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		rtpFrame.getContentPane().setLayout(null);

		JLabel lblNewLabel = new JLabel("Resident/Tenant Payment");
		lblNewLabel.setFont(new Font("Arial Rounded MT Bold", Font.BOLD, 37));
		lblNewLabel.setBounds(34, 11, 665, 81);
		rtpFrame.getContentPane().add(lblNewLabel);
		...
		rtpFrame.getContentPane().add(lblPay);
		...
		rtpFrame.getContentPane().add(txtPay);
				...
				rtpFrame.getContentPane().add(lblDate);
				...
				rtpFrame.getContentPane().add(txtDate);
		...
		rtpFrame.getContentPane().add(lblCard);

		JTextField txtCard = new JTextField();
		txtCard.setBounds(554, 173, 250, 42);
		rtpFrame.getContentPane().add(txtCard);
		...
		rtpFrame.getContentPane().add(lblPending);
		...
		rtpFrame.getContentPane().add(txtPending);
		...
		rtpFrame.getContentPane().add(errorText);
		...
		rtpFrame.getContentPane().add(scroll);
//		

		// Payment Display
		CRUD newCrud = new CRUD();
		tblDataHistory = newCrud.read("ResTenPaymentHistory.txt");
		...
		for (int i = 0; i < tblDataHistory.size(); i++) {
			...
			for (int b = 0; b < rowPayment.length; b++) {
				if (rowPayment[b].toLowerCase().contains(restenname.toLowerCase())) {
				match1 = true;
				break;
				}
			}
		...
		rtpFrame.getContentPane().add(scrollPayment);
		...
		rtpFrame.getContentPane().add(calErrorText);
		...
		addUserBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				crud.create("ResTenPaymentHistory.txt", data);
				...
				crud.update("ResTenPendingFee.txt", userID, 0, "", 0, String.valueOf(totalDebt), 3, "", 0);
				crud.update("ResTenPaymentHistory.txt", uid, 0,"", 0, String.valueOf(totalDebt), 5, "", 0);
				crud.update("ResTenPaymentHistory.txt", uid, 0,"", 0, String.valueOf(totalDebt), 3, "", 0);
			...
			for (int w = 0; w < row.length; w++) {
				if (row[w].toLowerCase().contains(restenname.toLowerCase())) {
				match2 = true;
				break;
		}
	}
			...
			if (match2) {
				...
				tblDataHistory = crud.read("ResTenPaymentHistory.txt");
				DefaultTableModel tableModelPayment = (DefaultTableModel) jTablePayment.getModel();
				tableModelPayment.setColumnIdentifiers(columnPayment);
		...
		rtpFrame.getContentPane().add(addUserBtn);
		...
		rtpFrame.getContentPane().add(viewReceiptBtn);
		...
		rtpFrame.getContentPane().add(backBtn);
